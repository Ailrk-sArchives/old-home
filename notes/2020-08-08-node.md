...
In the original paper of reactor pattern, the author assumes all IO operations are provided by `select` linux system call, so there is no need for threads. This not the case in node. Node need to perform more types of IO operations than those supported by `select` alone. For those asynchronous operations that are not directly supported by the hardware, node needs a thread pool to handle the IO in parallel in order to avoid blocking. After the thread finished the IO operation it will notify the event demultiplexer, and the returned value (`handle` in the original paper) will be dispatched with its corresponding callback and be pushed into the event queue.
